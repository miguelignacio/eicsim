...
ifeq ($(ROOT_USE),true)
	$(CXX) $< main92.so -o $@ -w -I$(FASTJET3_INCLUDE) -I$(ROOT_INCLUDE) $(CXX_COMMON)\
...
breit: $$@.cc $(PREFIX_LIB)/libpythia8.a breit.so
ifeq ($(ROOT_USE),true)
	$(CXX) $< breit.so -o $@ -w -I$(FASTJET3_INCLUDE) -I$(ROOT_INCLUDE) -I/mnt/c/Users/marratia/Linux/DIRE/include $(CXX_COMMON)\
	 `$(ROOTBIN)root-config --cflags` -Wl,-rpath,./\
	 -L$(FASTJET3_LIB) -Wl,-rpath,$(FASTJET3_LIB) -lfastjet\
	 -Wl,-rpath,$(ROOT_LIB) `$(ROOT_BIN)root-config --glibs`
else
...
breit.so: breitDct.cc $(PREFIX_LIB)/libpythia8.a
	$(CXX) $< -o $@ -w -I$(ROOT_INCLUDE) $(CXX_SHARED) $(CXX_COMMON)\
	 `$(ROOTBIN)root-config --cflags`
breitDct.cc: breit.h breitLinkDef.h
	export LD_LIBRARY_PATH=$$LD_LIBRARY_PATH:$(ROOT_LIB);\
	$(ROOT_BIN)rootcint -f $@ -c -I$(PREFIX_INCLUDE) $^
